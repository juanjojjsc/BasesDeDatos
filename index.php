			...
			if ($_POST['registrar'] == '1') {
				...
				if (mysqli_num_rows($result) > 0) {
					...
					if(!mysqli_query($link,$query)) {
						...
						//$usuario_actual = $_SESSION['id'];
						$queryID = "SELECT id_u FROM `users` WHERE email_u = '".mysqli_real_escape_string($link, $_POST['email'])."'";
						$IDactualArray = mysqli_fetch_array(mysqli_query($link,$queryID));
						$IDActual = $IDactualArray['id_u'];

						$query2 = "INSERT INTO `notas` (`id_u`,`contenido`,`creado`,`titulo`,`actualizado`) VALUES($IDActual,'',SYSDATE(),'',SYSDATE())";
					
						if(!mysqli_query($link,$query2)) {

							$error = "<p>Errorrr mirreeyy</p>";
							die($usuario_actual);
...
<?php include("header.php"); ?>
  	...
  	<div class="container" id="logInContainer">
  		...
  		<div id="fondo">
			...
			<div>
		<button type="button" class="logbutt" onclick="window.location='http://176.32.230.48/misnotastec.com/admin.php'">Admin</button>
		</div>
